...
func (cc *CosmosProvider) BuildAndBroadcast(ctx context.Context, msgs []provider.RelayerMessage, memo string) (*sdk.TxResponse, bool, error) {
	...
	if err != nil {
		...
		// Packet(s) already handled by another relayer
		if strings.Contains(err.Error(), chantypes.ErrRedundantTx.Error()) {
			cc.log.Info("Skipping retry due to redundant packet")
			return nil, false, err
		}
...
func (cc *CosmosProvider) AcknowledgementFromSequence(ctx context.Context,
	dst provider.ChainProvider, dsth, seq uint64, dstChanId, dstPortId,
	srcChanId, srcPortId string,
) (provider.RelayerMessage, error) {
	// query the transaction as received on the destination and generated the ack
	...
	for _, ack := range acks {
		...
		// query the ack as generated on dst and build a message with proof to be sent to dst
...
func (cc *CosmosProvider) NewClientState(
	...
	if dstUpdateHeader.ClientType() == ibcexported.Furyint {
...
func NewClientHeader(
	...
	if clientType == ibcexported.Furyint {
		return &types.Header{
...
func GetTrustedHeight(header ibcexported.Header) (*clienttypes.Height, error) {
	if header.ClientType() == ibcexported.Furyint {

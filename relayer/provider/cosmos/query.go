...
func (cc *CosmosProvider) QueryBalanceWithAddress(ctx context.Context, address string) (sdk.Coins, error) {
	...
	for {
		...
		total = append(total, res.Balances...)
...
func (cc *CosmosProvider) QueryClients(ctx context.Context) (clienttypes.IdentifiedClientStates, error) {
	...
	total := []clienttypes.IdentifiedClientState{}
	pagination := DefaultPageRequest()
	...
	for {
		...
		total = append(total, res.ClientStates...)
		if len(res.Pagination.NextKey) == 0 {
			break
		}
		pagination = DefaultPageRequest()
...
func (cc *CosmosProvider) QueryConnections(ctx context.Context) (conns []*conntypes.IdentifiedConnection, err error) {
	...
	total := []*conntypes.IdentifiedConnection{}
	pagination := DefaultPageRequest()
	...
	for {
		...
		total = append(total, res.Connections...)
		if len(res.Pagination.NextKey) == 0 {
			break
		}
		pagination = DefaultPageRequest()
...
func (cc *CosmosProvider) QueryConnectionsUsingClient(ctx context.Context, height int64, clientid string) ([]*conntypes.IdentifiedConnection, error) {
	qc := conntypes.NewQueryClient(cc)
	...
	for {
		res, err := qc.Connections(ctxWithHeight, &conntypes.QueryConnectionsRequest{
			Pagination: pagination,
		})
		if err != nil {
			return nil, err
...
func (cc *CosmosProvider) QueryChannelClient(ctx context.Context, height int64, channelid, portid string) (*clienttypes.IdentifiedClientState, error) {
	...
	ctxWithHeight := lens.SetHeightOnContext(ctx, int64(height))
	cState, err := qc.ChannelClientState(ctxWithHeight, &chantypes.QueryChannelClientStateRequest{
		PortId:    portid,
		ChannelId: channelid,
	})
	if err != nil {
...
func (cc *CosmosProvider) QueryChannels(ctx context.Context) ([]*chantypes.IdentifiedChannel, error) {
	qc := chantypes.NewQueryClient(cc)
	total := []*chantypes.IdentifiedChannel{}
	pagination := DefaultPageRequest()
	...
	for {
		...
		total = append(total, res.Channels...)
		if len(res.Pagination.NextKey) == 0 {
			break
		}
		pagination = DefaultPageRequest()
...
func (cc *CosmosProvider) QueryPacketCommitments(ctx context.Context, height uint64, channelid, portid string) (commitments []*chantypes.PacketState, err error) {
	qc := chantypes.NewQueryClient(cc)
	ctxWithHeight := lens.SetHeightOnContext(ctx, int64(height))
...
func (cc *CosmosProvider) QueryUnreceivedPackets(ctx context.Context, height uint64, channelid, portid string, seqs []uint64) ([]uint64, error) {
	qc := chantypes.NewQueryClient(cc)
	ctxWithHeight := lens.SetHeightOnContext(ctx, int64(height))
	res, err := qc.UnreceivedPackets(ctxWithHeight, &chantypes.QueryUnreceivedPacketsRequest{
		PortId:                    portid,
		ChannelId:                 channelid,
...
func (cc *CosmosProvider) QueryUnreceivedAcknowledgements(ctx context.Context, height uint64, channelid, portid string, seqs []uint64) ([]uint64, error) {
	qc := chantypes.NewQueryClient(cc)
	ctxWithHeight := lens.SetHeightOnContext(ctx, int64(height))
	res, err := qc.UnreceivedAcks(ctxWithHeight, &chantypes.QueryUnreceivedAcksRequest{
		PortId:             portid,
		ChannelId:          channelid,
...
func (cc *CosmosProvider) QueryPacketCommitment(ctx context.Context, height int64, channelid, portid string, seq uint64) (comRes *chantypes.QueryPacketCommitmentResponse, err error) {
	...
	if len(value) == 0 {
		fmt.Printf("query height+1 for %s, height %d", cc.ChainId(), height)
		value, proofBz, proofHeight, err = cc.QueryTendermintProof(ctx, height+1, key)
		if err != nil {
			return nil, err
		}
		if len(value) == 0 {
...
// QueryLatesFainalizedtHeight returns the finalized heights of a chain
func (cc *CosmosProvider) QueryLatestHeight(ctx context.Context) (srch int64, err error) {
	srcFinalizedStateH := int64(-1)

	eg, egCtx := errgroup.WithContext(ctx)
	eg.Go(func() error {
		var err error
		srch, err = cc.queryLatestHeight(egCtx)
		return err
	})
	if cc.ClientType() == ibcexported.Furyint {
		eg.Go(func() error {
			var err error
			srcFinalizedStateH, err = GetLatestFinalizedStateHeight(egCtx, cc.ChainId())
			return err
		})
	}
	if err = eg.Wait(); err != nil {
	...
	if srch == 0 {
		return srch, fmt.Errorf("failed to query latest heights, return zero")
	}

	return
}

// QueryLatestHeight returns the height a chain
...
func (cc *CosmosProvider) QueryDenomTraces(ctx context.Context, offset, limit uint64, height int64) ([]transfertypes.DenomTrace, error) {
	...
	ctxWithHeight := lens.SetHeightOnContext(ctx, int64(height))
	total := []transfertypes.DenomTrace{}
	pagination := DefaultPageRequest()

	for {
		res, err := qc.DenomTraces(ctxWithHeight, &transfertypes.QueryDenomTracesRequest{
			Pagination: pagination,
		})
		if err != nil {
			return nil, err
		}
		total = append(total, res.DenomTraces...)
		if len(res.Pagination.NextKey) == 0 {
			break
		}
		pagination = DefaultPageRequest()
